...
contains
    ...
    impure subroutine s_save_data(t_step, varname, pres, c, H)
        ...
        if (q_filtered_wrt) then
            ...
            do i = 1, 6
                do k = 1, 4
                    write (varname, '(A,I0,A,I0)') 'stat_reynolds_stress', i, '_m', k
                    call s_write_field(varname, t_step, stat_reynolds_stress(i)%vf(k), x_beg, x_end, y_beg, y_end, z_beg, z_end)
                end do
            end do
            do i = 1, 6
                do k = 1, 4
                    write (varname, '(A,I0,A,I0)') 'stat_eff_visc', i, '_m', k
                    call s_write_field(varname, t_step, stat_eff_visc(i)%vf(k), x_beg, x_end, y_beg, y_end, z_beg, z_end)
                end do
            end do
            do i = 1, num_dims
                do j = 1, 4
                    write (varname, '(A,I0,A,I0)') 'stat_int_mom_exch', i, '_m', j
                    call s_write_field(varname, t_step, stat_int_mom_exch(i)%vf(j), x_beg, x_end, y_beg, y_end, z_beg, z_end)
                end do
            end do
            do i = 1, eqn_idx%E
                do j = 1, 4
                    write (varname, '(A,I0,A,I0)') 'stat_q_cons_filtered', i, '_m', j
                    call s_write_field(varname, t_step, stat_q_cons_filtered(i)%vf(j), x_beg, x_end, y_beg, y_end, z_beg, z_end)
                end do
            end do
            do i = 1, 4
                write (varname, '(A,I0)') 'stat_filtered_pressure_m', i
                call s_write_field(varname, t_step, stat_filtered_pressure(i), x_beg, x_end, y_beg, y_end, z_beg, z_end)
            end do
        end if
